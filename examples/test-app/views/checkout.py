...
def create_checkout_page():
    ...
    with ui.card().classes("w-full max-w-2xl mx-auto"):
        ...
        with ui.form().classes("w-full"):
            # Card number
            card_number = ui.input(label="Card Number", placeholder="1234 5678 9012 3456").classes(
                "w-full mb-4"
            )
            ...
            with ui.row().classes("w-full gap-4"):
                expiry_month = ui.select(
                    label="Month", options=[f"{i:02d}" for i in range(1, 13)]
                ).classes("w-1/2")

                expiry_year = ui.select(
                    label="Year", options=[str(i) for i in range(2024, 2035)]
                ).classes("w-1/2")

            # CVV
            cvv = ui.input(label="CVV", placeholder="123", type="password").classes("w-full mb-4")
            ...
            ui.button(
                ...
                on_click=lambda: handle_purchase(
                    card_number.value, f"{expiry_month.value}/{expiry_year.value}", cvv.value
                ),
            ).classes("w-full")

    # Back to cart button
    ui.button("Back to Cart", on_click=lambda: ui.navigate.to("/cart")).classes("mt-4")
...
def complete_purchase():
    ...
    track_event("purchase_completed", None, {"amount": 99.99, "currency": "USD"})

...
# Simulated database
users = {"test@example.com": {"password": "password123", "name": "Test User"}}
...
def toggle_error(flow):
    ...
    if flow in error_states:
        ...
        return {"success": True, "flow": flow, "error_state": error_states[flow]}
...
def random_error():
    ...
    return {"success": True, "flow": flow, "error_state": True}
    ...
    with ui.card().classes("fixed top-4 right-4 z-10 bg-gray-800 p-4 rounded-lg shadow-lg"):
        ...
        for flow in error_states:
            with ui.row().classes("items-center gap-2 mb-2"):
                ...
                ui.switch(
                    value=error_states[flow], on_change=lambda e, f=flow: toggle_error(f)
                ).classes("text-white")

        # Random error button
        ui.button("Trigger Random Error", on_click=random_error).classes(
            "w-full mt-4 bg-red-500 hover:bg-red-600 text-white"
        )
        ...
        with ui.row().classes("w-full justify-between items-center"):
            ...
            with ui.row().classes("items-center gap-4"):
                with (
                    ...
                    .props("flat")
                    .classes("text-white") as menu_button
                ):
                    with ui.menu().classes("bg-gray-800 text-white") as menu:
                        if current_user:
                            ui.menu_item(
                                "Store", on_click=lambda: ui.navigate.to("/store")
                            ).classes("hover:bg-gray-700")
                            ui.menu_item("Cart", on_click=lambda: ui.navigate.to("/cart")).classes(
                                "hover:bg-gray-700"
                            )
                            ui.menu_item(
                            ...
                            ui.menu_item("Logout", on_click=lambda: set_current_user(None)).classes(

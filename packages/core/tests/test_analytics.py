...
def test_flow_scorer_get_business_criticality(config_file):
    """Test getting business criticality for pages."""
    scorer = FlowScorer(str(config_file))
    assert scorer.get_business_criticality("/login") == 5
    assert scorer.get_business_criticality("/unknown") == 2  # default value
...
def test_flow_scorer_calculate_score(tmp_path):
    ...
    assert result["score"] == 10 + (1 * 2.0) + (5 * 1.5)  # frequency + (errors * error_weight) + (criticality * business_weight)
...
def test_flow_scorer_properties(flow, config):
    ...
    # Business criticality should be from config
    for action in flow["actions"]:
        if action["type"] == "[Amplitude] Page Viewed":
            page = action["data"]["[Amplitude] Page URL"].split('/')[-1]
            assert result["business_criticality"] == config["business_criticality"].get(page, config["business_criticality"]["default"])
            break
...
def test_flow_scorer_edge_cases(data):
    ...
    config = {
        "weights": {
            ...
            "business_weight": data.draw(st.floats(min_value=0.1, max_value=5.0))
        },
        "business_criticality": {
            "default": data.draw(st.integers(min_value=1, max_value=5))
        }
    }
    ...
    # Test flow with no page views
    no_page_flow = {
        "frequency": 1,
        "actions": [{"type": "error", "data": {"message": "Test"}}]
    }
    result = scorer.calculate_score(no_page_flow)
    assert result["business_criticality"] == 0
...
def test_flow_scorer_page_criticality(data):
    """Test business criticality for different pages."""
    # Generate random config with known pages
    pages = ["login", "checkout", "profile", "settings"]
    criticality = {
        "default": data.draw(st.integers(min_value=1, max_value=5))
    }
    for page in pages:
        criticality[page] = data.draw(st.integers(min_value=1, max_value=5))
    ...
    for page in pages:
        flow = {
            ...
            "actions": [{
                "type": "[Amplitude] Page Viewed",
                "data": {"[Amplitude] Page URL": f"/{page}"}
            }]
        }
        result = scorer.calculate_score(flow)
        assert result["business_criticality"] == criticality[page]
    ...
    result = scorer.calculate_score(flow)
    assert result["business_criticality"] == criticality["default"]
...
def test_flow_scorer_config_generation(tmp_path):
    ...
    # Verify config
    assert config["weights"]["error_weight"] == 1.5  # Adjusted based on error frequency
    assert "login" in config["business_criticality"]
    assert config["business_criticality"]["login"] == 5  # Default criticality for login 